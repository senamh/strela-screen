...
// Fit the actual crop, including a protected area wider than the requested output.
export function containCrop(g){const fit=Math.min(g.w/g.crop.w,g.h/g.crop.h),w=g.crop.w*fit,h=g.crop.h*fit;return {...g,x:g.x+(g.w-w)/2,y:g.y+(g.h-h)/2,w,h};}
export function render(canvas,source,sw,sh,p,t){
  ...
  if(s.ratio==='phone'){
    const {overview,detail:slot}=phoneLayout(W,H,sw,sh),points=p.points.filter(point=>timelineTime(clips,point.t)!==null);
    ...
    const detail=containCrop({...slot,crop:camera(points,t,sw,sh,s,slot.w/slot.h,clips,track)});
    for(const g of [overview,detail]){ctx.save();ctx.beginPath();ctx.roundRect(g.x,g.y,g.w,g.h,g===overview||s.keepArea?0:Math.min(s.radius*scale,g.w/2,g.h/2));ctx.clip();if(source)ctx.drawImage(source,g.crop.x,g.crop.y,g.crop.w,g.crop.h,g.x,g.y,g.w,g.h);ctx.restore();}
    return {...detail,overview};
  }
  const pad=s.padding*scale,fit=Math.min((W-pad*2)/sw,(H-pad*2)/sh),slotW=s.ratio==='portrait'?W-pad*2:sw*fit,slotH=s.ratio==='portrait'?H-pad*2:sh*fit;
  const {x,y,w,h,crop}=containCrop({x:(W-slotW)/2,y:(H-slotH)/2,w:slotW,h:slotH,crop:camera(p.points,t,sw,sh,s,slotW/slotH,clips,track)}),r=s.keepArea?0:Math.min(s.radius*scale,w/2,h/2);
