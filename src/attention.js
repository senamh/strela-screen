...
export function attention(previous,current,width,height){
  const cols=24,rows=16,weights=new Float64Array(cols*rows);let changed=0;
  ...
  const fraction=changed/(width*height);if(fraction<.0015||fraction>.22)return null;
