...
test('reopening the popup updates the reused studio source and focuses its window',async()=>{
  const button={},buildLabel={},calls=[],message={};let closed=false;
  ...
  vm.runInNewContext(await readFile('src/popup.js','utf8'),{chrome,__STRELA_BUILD_LABEL__:'0.6.0 · fixture',document:{querySelector:selector=>selector==='#build-info'?buildLabel:button},window:{close:()=>closed=true}});
  assert.equal(buildLabel.textContent,'0.6.0 · fixture');
